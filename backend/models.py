...
from sqlalchemy import (
    ...
    ForeignKey, Boolean, JSON, Enum, Index, Table
)
...
class User(Base):
    ...
    acknowledged = relationship("AlertHistory",
...
class Patient(Base):
    ...
    alert_history = relationship("AlertHistory", back_populates="patient", cascade="all, delete-orphan")
...
class Alert(Base):
    """
    Stores active/unacknowledged clinical alerts.
    Deleted upon acknowledgement (moved to history), cancellation, or 24h expiration.
    """
...
# ── AlertHistory (Permanent History) ──────────────────────────────────────────
class AlertHistory(Base):
    """
    Permanent clinical alert logs.
    Contains only acknowledged alerts, ordered chronologically.
    """
    __tablename__ = "alert_history"

    id              = Column(Integer, primary_key=True, index=True)
    alert_id        = Column(Integer, nullable=False)    # Original alert ID
    patient_id      = Column(Integer, ForeignKey("patients.id"), nullable=False)
    ...
    created_at      = Column(DateTime, nullable=False)    # Original creation timestamp
    status          = Column(String(20), default="ACKNOWLEDGED", nullable=False)

    patient      = relationship("Patient", back_populates="alert_history")
...
Index("idx_alert_history_patient",  AlertHistory.patient_id, AlertHistory.created_at)
...
class Room(Base):
    """
    Represents a physical room in the hospital ward.
    Tracks assigned patients, nurses, and enabled monitoring services.
    """
    __tablename__ = "rooms"
    ...
    monitoring_status = Column(String(50), default="Idle", nullable=False)
    created_at        = Column(DateTime, server_default=func.now())
    updated_at        = Column(DateTime, server_default=func.now(), onupdate=func.now())

    # Relationships
    patient  = relationship("Patient", backref="room", uselist=False)
    services = relationship("RoomService", back_populates="room", cascade="all, delete-orphan")
    nurses   = relationship("User", secondary=room_nurses, backref="rooms")


class RoomService(Base):
    """
    Enabled monitoring services for a room (e.g. 'ecg', 'seizure', 'fall').
    """
