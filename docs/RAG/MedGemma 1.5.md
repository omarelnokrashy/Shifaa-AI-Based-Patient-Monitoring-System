> [!NOTE]
> **Note:** This card describes MedGemma 1.5, which is only available as a 4B multimodal instruction-tuned variant. For information on MedGemma 1 variants, refer to the [MedGemma 1 model
...
- [GitHub repository](https://github.com/google-health/medgemma):
  ...
  - [Custom container source code](https://github.com/Google-Health/medgemma/tree/main/python) for server-side image processing. You can find it prebuilt as a [Docker
    image](http://us-docker.pkg.dev/deeplearning-platform-release/vertex-model-garden/health-ai-medgemma.1-0) based on a [custom
    architecture](https://developers.google.com/health-ai-developer-foundations/model-serving/overview) with a [convenient
    API](https://developers.google.com/health-ai-developer-foundations/medgemma/serving-api). It is used to provide additional deployment options in Model Garden alongside the standard [vLLM container](https://github.com/vllm-project/vllm).
...
> [!NOTE]
> **Note:** If you need to use the model at scale, we recommend creating a production version using [Model
> Garden](https://console.cloud.google.com/vertex-ai/publishers/google/model-garden/medgemma). Model Garden provides various deployment options and tutorial notebooks, including specialized server-side image processing options for efficiently handling large medical images: Whole Slide Digital Pathology (WSI) or volumetric scans (CT/MRI) stored in [Cloud DICOM
> Store](https://docs.cloud.google.com/healthcare-api/docs/concepts/dicom) or [Google Cloud Storage (GCS)](https://cloud.google.com/storage).
...
- **Key publication** : <https://arxiv.org/abs/2604.05081>
- **Model created** : **4B multimodal**: Jan 13, 2026
- **Model version** : **4B multimodal**: 1.5.0

### Citation

When using this model, please cite: Sellergren et al. "MedGemma 1.5 Technical
Report." *arXiv preprint arXiv:2604.05081* (2026).

    @article{sellergren2026medgemmatechnicalreport,
      title={MedGemma 1.5 Technical Report},
      author={Sellergren, Andrew and Gao, Chufan and Mahvar, Fereshteh and Kohlberger, Timo and Jamil, Fayaz and Traverse, Madeleine and Tono, Alberto and Sadjad, Bashir and Yang, Lin and others},
...
- [MS-CXR-T](https://physionet.org/content/ms-cxr-t/1.0.0/): Microsoft Research Health Futures, Microsoft Research.
- [ChestX-ray14](https://pmc.ncbi.nlm.nih.gov/articles/PMC6476887/): National Institutes of Health - Clinical Center.
...
- [PMC-OA (PubMed Central Open Access
  Subset)](https://catalog.data.gov/dataset/pubmed-central-open-access-subset-pmc-oa): Maintained by the National Library of Medicine (NLM) and National Center for Biotechnology Information (NCBI), which are part of the NIH.
...
- [MedMCQA](https://arxiv.org/abs/2203.14371): This dataset was created by Ankit Pal, Logesh Kumar Umapathi and Malaikannan Sankarasubbu from Saama AI Research, Chennai, India
- [PubMedQA](https://arxiv.org/abs/1909.06146): This dataset was created by Qiao Jin, Bhuwan Dhingra, Zhengping Liu, William W. Cohen, Xinghua Lu from the University of Pittsburg, Carnegie Mellon University and Google.
- [LiveQA](https://trec.nist.gov/pubs/trec26/papers/Overview-QA.pdf): This dataset was created by Ben Abacha Asma, Eugene Agichtein Yuval Pinter and Dina Demner-Fushman from the U.S. National Library of Medicine, Emory University and Georgia Institute of Technology.
- [Mendeley Digital Knee
  X-Ray](https://data.mendeley.com/datasets/t9ndx37v5h/1): This dataset is from Rani Channamma University, and is hosted on Mendeley Data.
...
- [ISIC](https://www.isic-archive.com/): International Skin Imaging Collaboration is a joint effort involving clinicians, researchers, and engineers from various institutions worldwide.
- [Mendeley Clinical Laboratory Test
  Reports](https://data.mendeley.com/datasets/bygfmk4rx9/2): This dataset is hosted on Mendeley and includes 260 clinical laboratory test reports issued by 24 laboratories in Egypt.
- [CT-RATE](https://huggingface.co/datasets/ibrahimhamamci/CT-RATE): Istanbul Medipol University Mega Hospital and University of Zurich / ETH Zurich.
...
- **MedMCQA:** Pal, Ankit, Logesh Kumar Umapathi, and Malaikannan Sankarasubbu. "Medmcqa: A large-scale multi-subject multi-choice dataset for medical domain question answering." *Conference on health, inference, and
  learning. PMLR,* 2022.
- **PubMedQA:** Jin, Qiao, et al. "Pubmedqa: A dataset for biomedical research question answering." *Proceedings of the 2019 conference on empirical
