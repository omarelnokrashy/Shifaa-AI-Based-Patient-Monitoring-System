...
run_test_suite "Retriever Unit Tests" "tests/integration/test_retriever.py"
...
if curl -sf "${BASE_URL:-http://localhost:8000}/" > /dev/null 2>&1; then
    ...
    run_test_suite "API Integration Tests" "tests/integration/test_api.py"
else
...
if [ -z "$SKIP_LLM" ]; then
    ...
    run_test_suite "Intent Classification Tests" "tests/integration/test_intent.py"
    run_test_suite "NER Extraction Tests" "tests/integration/test_ner.py"
else
    echo ""
...
if [ -n "$SKIP_LLM" ]; then
    python tests/integration/benchmark_pipeline.py --skip-llm --output "$REPORT_FILE" 2>&1 | tee -a "$LOG_FILE"
else
    python tests/integration/benchmark_pipeline.py --output "$REPORT_FILE" 2>&1 | tee -a "$LOG_FILE"
fi
